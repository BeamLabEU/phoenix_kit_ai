defmodule PhoenixKitAI.ActivityLogAssertions do
  @moduledoc """
  Helpers for asserting activity log entries landed with the right
  action, module, and metadata shape.

  Imported into `PhoenixKitAI.DataCase` and `PhoenixKitAI.LiveCase` so
  every DB-backed test can reach them.

  ## Why direct SQL?

  We bypass the `PhoenixKit.Activity` Ecto schema and query the
  `phoenix_kit_activities` table directly so the helper works without
  pulling that schema into the test scope. Postgres returns UUIDs as
  raw 16-byte binaries — `uuid_match?/2` normalises both sides to the
  textual form so callers can pass plain UUID strings.
  """
